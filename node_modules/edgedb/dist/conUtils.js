    ...
    if (!hasCompoundOptions) {
        ...
        if (instName !== null) {
            const cloudProfile = await serverUtils
                ...
                .then(name => name.trim())
                .catch(() => undefined);
            await resolveConfigOptions(resolvedConfig, { instanceName: instName, cloudProfile }, {
...
async function resolveConfigOptions(resolvedConfig, config, sources, compoundParamsError, serverUtils) {
    ...
    anyOptionsUsed =
        resolvedConfig.setPassword((_d = config.password) !== null && _d !== void 0 ? _d : null, sources.password) ||
            anyOptionsUsed;
    anyOptionsUsed =
        resolvedConfig.setSecretKey((_e = config.secretKey) !== null && _e !== void 0 ? _e : null, sources.secretKey) || anyOptionsUsed;
    anyOptionsUsed =
...
async function parseCloudInstanceNameIntoConfig(config, cloudInstanceName, source, serverUtils) {
    ...
    try {
        ...
        const dnsBucket = ((0, crcHqx_1.crcHqx)(buffer_1.utf8Encoder.encode(cloudInstanceName), 0) % 100)
            .toString(10)
            .padStart(2, "0");
